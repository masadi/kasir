...
def create_access_token(uid: str, email: str, role: str, owner_id: str) -> str:
    payload = {
        "sub": uid, "email": email, "role": role, "owner_id": owner_id,
...
class ProductIn(BaseModel):
    ...
    hpp: float = 0
    stock: float = 0
    unit: str = "pcs"  # pcs, kg, ons, botol, bungkus
...
async def get_store(user: dict = Depends(get_current_user)):
    ...
    if not s:
        s = {"owner_id": oid, "shop_name": "", "address": "", "owner_wa": "",
             "qris_image_path": None, "receipt_footer": "Terima kasih!", "onboarded": False}
        await db.stores.insert_one(s.copy())
    return s

@api.put("/store")
async def update_store(payload: StoreSettings, user: dict = Depends(require_owner)):
    oid = user["id"]
    data = payload.model_dump(exclude_unset=True)
    await db.stores.update_one({"owner_id": oid}, {"$set": data}, upsert=True)
    s = await db.stores.find_one({"owner_id": oid}, {"_id": 0})
    return s

@api.post("/store/complete-onboarding")
async def complete_onboarding(user: dict = Depends(require_owner)):
    await db.stores.update_one({"owner_id": user["id"]}, {"$set": {"onboarded": True}})
    return {"ok": True}
...
async def upload(file: UploadFile = File(...), user: dict = Depends(get_current_user)):
    ...
    await db.files.insert_one({
        "id": str(uuid.uuid4()), "owner_id": oid,
...
async def download(path: str, auth: Optional[str] = Query(None), authorization: Optional[str] = Header(None)):
    # Auth: allow either header or ?auth= query for img tags
    token = None
    ...
    # Multi-tenant isolation: file owner_id must match caller's owner_id
...
async def list_products(user: dict = Depends(get_current_user)):
    oid = owner_id_of(user)
    items = await db.products.find({"owner_id": oid, "is_deleted": {"$ne": True}}, {"_id": 0}).to_list(1000)
    return items
...
async def delete_product(pid: str, user: dict = Depends(require_owner)):
    await db.products.update_one({"id": pid, "owner_id": user["id"]}, {"$set": {"is_deleted": True}})
    return {"ok": True}

# ---------- Cashiers ----------
@api.get("/cashiers")
async def list_cashiers(user: dict = Depends(require_owner)):
    items = await db.users.find({"owner_id": user["id"], "role": "cashier"}, {"_id": 0, "password_hash": 0}).to_list(200)
    return items
...
async def create_txn(t: TxnIn, user: dict = Depends(get_current_user)):
    ...
    # Decrement stock atomically and calculate profit
    profit = 0.0
    for it in t.items:
        prod = await db.products.find_one_and_update(
            {"id": it.product_id, "owner_id": oid},
    ...
    doc = {
        ...
        "items": [it.model_dump() for it in t.items],
        "total": t.total, "payment_method": t.payment_method,
        "cash_received": t.cash_received, "change": t.change,
        "profit": profit, "created_at": now.isoformat()
    }
...
async def summary(user: dict = Depends(get_current_user)):
    ...
    async def agg(start_iso):
        ...
        r = await db.transactions.aggregate(pipe).to_list(100)
        cash = next((x for x in r if x["_id"] == "cash"), None)
        qris = next((x for x in r if x["_id"] == "qris"), None)
        return {
            "cash_total": (cash or {}).get("total", 0),
            "qris_total": (qris or {}).get("total", 0),
            "total": sum(x["total"] for x in r),
            "count": sum(x["count"] for x in r),
    ...
    return {"today": today, "week": week, "series": series, "low_stock": low}
...
async def on_start():
    ...
    if not existing:
        ...
        await db.stores.insert_one({
            "owner_id": uid, "shop_name": "Warung Saya", "address": "",
            "owner_wa": "", "qris_image_path": None,
            "receipt_footer": "Terima kasih!", "onboarded": False
        })
...
# CORS - allow credentials with dynamic origin reflection
